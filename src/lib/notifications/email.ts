...
export type AdminNotificationType = "approved" | "rejected" | "requested_changes" | "archived" | "unpublished" | "restored";
...
export async function sendAdminNotification(notification: AdminNotification) {
  ...
  const action = notification.type.replace("_", " ");
  const text = [
    `Your ChatScout community "${notification.communityName}" was ${action}.`,
    notification.note ? `Review note: ${notification.note}` : null,
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from,
      to: notification.to,
      subject: `ChatScout update: ${notification.communityName}`,
      text,
    }),
  });
