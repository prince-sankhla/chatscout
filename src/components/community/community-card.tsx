...
function verificationLabel(status: Community["verificationStatus"]) {
...
export function CommunityCard({ community, rank, compact = false }: { community: Community; rank?: number; compact?: boolean }) {
  return <article className={`community-card ${compact ? "community-card-compact" : ""}`}>
    <Link className={`community-art art-${community.accent}`} href={`/community/${community.slug}`} aria-label={`View ${community.name}`}>
      ...
      {community.verificationStatus === "verified" && <span className="community-verified-badge"><Icon name="check" size={12} /> Verified</span>}
    </Link>
    <div className="community-info">
      <div className="community-title-row"><h3><Link href={`/community/${community.slug}`}>{community.name}</Link></h3></div>
      <div className="tag-row">{community.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>
      <div className="community-trust-row"><span><Icon name="users" size={15} />{community.membersLabel}</span><span>{verificationLabel(community.verificationStatus)}</span></div>
      <div className="community-trust-row secondary"><span>{community.healthLabel}</span><span>{community.listingAgeLabel}</span></div>
