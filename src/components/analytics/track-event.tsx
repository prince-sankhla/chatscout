...
export function TrackEvent({ eventName, communityId, categoryId, dedupeKey, metadata: extra = {} }: Props) {
  useEffect(() => {
    const searchFingerprint = eventName === "search" ? window.location.search : "";
    const baseFingerprint = `${eventName}:${window.location.pathname}:${communityId ?? categoryId ?? ""}`;
    const key = `cs_tracked:${dedupeKey ?? `${baseFingerprint}:${searchFingerprint}`}`;
    try {
