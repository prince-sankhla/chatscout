...
export async function getPublishedCommunities(filters: PublishedCommunityFilters = {}): Promise<CommunityQueryResult<CommunityRow[]>> { const supabase = createServerSupabaseClient(); const category = await communityIdsForCategory(filters.categorySlug); if (category.error) return queryFailure(category.error, "Unable to load communities."); if (category.ids && category.ids.length === 0) return { data: [], error: null }; let query = supabase.from("communities").select("*").eq("status", "published").order(orderColumn(filters.sort), { ascending: false, nullsFirst: false }); if (filters.sort === "members") query = query.order("published_at", { ascending: false }); if (filters.platform) query = query.eq("platform", filters.platform); if (category.ids) query = query.in("id", category.ids); const { data, error } = await query.limit(MAX_SEARCH_RESULTS); if (error) return queryFailure(error, "Unable to load communities."); return { data: applyDiscoveryFilters(data ?? [], filters), error: null }; }
...
export async function getPublishedRelatedCommunities(communityId: string, limit = 6): Promise<CommunityQueryResult<CommunityRow[]>> { const supabase = createServerSupabaseClient(); const { data: sourceLinks, error: sourceError } = await supabase.from("community_categories").select("category_id").eq("community_id", communityId); if (sourceError) return queryFailure(sourceError, "Unable to load related communities."); const categoryIds = [...new Set(sourceLinks.map((link) => link.category_id))]; if (!categoryIds.length) return { data: [], error: null }; const { data: links, error: linkError } = await supabase.from("community_categories").select("community_id").in("category_id", categoryIds).neq("community_id", communityId); if (linkError) return queryFailure(linkError, "Unable to load related communities."); const ids = [...new Set(links.map((link) => link.community_id))]; if (!ids.length) return { data: [], error: null }; const { data, error } = await supabase.from("communities").select("*").eq("status", "published").in("id", ids).order("published_at", { ascending: false }).limit(limit); if (error) return queryFailure(error, "Unable to load related communities."); return { data: data ?? [], error: null }; }
export async function searchPublishedCommunities(term: string, filters: Omit<PublishedCommunityFilters, "platform"> & { platform?: CommunityPlatform } = {}): Promise<CommunityQueryResult<CommunityRow[]>> { const searchTerm = normalizeSearchTerm(term); if (!searchTerm) return getPublishedCommunities(filters); const category = await communityIdsForCategory(filters.categorySlug); if (category.error) return queryFailure(category.error, "Unable to search communities."); if (category.ids && category.ids.length === 0) return { data: [], error: null }; const supabase = createServerSupabaseClient(), pattern = `%${searchTerm}%`; const [{ data: textMatches, error: textError }, { data: categories, error: categoryError }] = await Promise.all([supabase.from("communities").select("*").eq("status", "published").or(`name.ilike.${pattern},description.ilike.${pattern}`).limit(MAX_SEARCH_RESULTS), supabase.from("categories").select("id").eq("is_active", true).ilike("name", pattern)]); if (textError) return queryFailure(textError, "Unable to search communities."); if (categoryError) return queryFailure(categoryError, "Unable to search communities."); let categoryMatches: CommunityRow[] = []; if (categories.length) { const { data: categoryLinks, error: linkError } = await supabase.from("community_categories").select("community_id").in("category_id", categories.map((category) => category.id)); if (linkError) return queryFailure(linkError, "Unable to search communities."); const ids = [...new Set(categoryLinks.map((link) => link.community_id))]; if (ids.length) { const { data, error } = await supabase.from("communities").select("*").eq("status", "published").in("id", ids).limit(MAX_SEARCH_RESULTS); if (error) return queryFailure(error, "Unable to search communities."); categoryMatches = data ?? []; } } let matches = [...new Map([...textMatches, ...categoryMatches].map((community) => [community.id, community])).values()]; if (filters.categorySlug) { const allowed = new Set(category.ids ?? []); matches = matches.filter((community) => allowed.has(community.id)); } if (filters.platform) matches = matches.filter((community) => community.platform === filters.platform); matches = applyDiscoveryFilters(matches, filters); matches.sort((a, b) => filters.sort === "members" ? (b.member_count ?? -1) - (a.member_count ?? -1) : new Date(b.published_at ?? b.created_at).getTime() - new Date(a.published_at ?? a.created_at).getTime()); return { data: matches.slice(0, MAX_SEARCH_RESULTS), error: null }; }
