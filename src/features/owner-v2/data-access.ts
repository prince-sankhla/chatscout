...
export async function getOwnerRewardsData(): Promise<OwnerRewardsData | null> {
  ...
  const { data: communities } = await admin.from("communities").select("id,name,platform,claim_status,verification_status,health_status,status").eq("owner_user_id", user.id).order("created_at", { ascending: false });
