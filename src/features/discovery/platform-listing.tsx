...
export async function PlatformListing({ kind, query = "" }: { kind: ListingKind; query?: string }) {
  ...
  return <main className="platform-page"><section className="platform-heading"><Link href="/" className="back-link">← Back to discovery</Link><p className="eyebrow">CHATSCOUT DISCOVERY</p><h1>{title}</h1><p>{subtitle}</p>{kind === "search" && <form className="neon-search platform-search" action="/search"><Icon name="search" /><input name="q" defaultValue={query} placeholder="Search Instagram group chats" /><button type="submit">Search</button></form>}</section>{result.error ? <p className="neon-empty">Communities are temporarily unavailable. Please try again later.</p> : communities.length ? <CommunityGrid communities={communities} /> : <p className="neon-empty">No published communities match this view yet.</p>}</main>;
}
