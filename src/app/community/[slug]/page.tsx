...
export default async function CommunityPage({ params }: PageProps) {
  ...
  return <PageShell><main className="page-content detail-page">
    ...
    <section className="detail-hero detail-hero-polished detail-hero-compact">
      ...
      <div className="detail-copy detail-copy-polished"><div className="detail-label-row"><p className="eyebrow">INSTAGRAM COMMUNITY</p><span className="platform-badge"><Icon name="instagram" size={13} />Instagram</span></div><h1>{community.name}</h1><div className="tag-row">{community.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><p className="detail-description">{community.description}</p>{metadata.length > 0 && <div className="detail-metadata">{metadata.map((item) => <span key={item.label}><Icon name={item.icon} size={16} />{item.label}</span>)}</div>}{guidelines.length > 0 && <dl className="detail-guidelines">{guidelines.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>}<Link className="join-button detail-join" href={`/join/${community.slug}`}><Icon name="instagram" size={18} />Join on Instagram <Icon name="arrow" size={16} /></Link><CommunityDetailActions slug={community.slug} name={community.name} /></div>
    </section>
