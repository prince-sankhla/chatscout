...
export default async function CommunityPage({ params }: PageProps) {
  ...
  return <PageShell><main className="page-content detail-page">
    ...
    <section className="detail-hero detail-hero-polished detail-hero-compact">
      ...
      <div className="detail-copy detail-copy-polished"><div className="detail-label-row"><p className="eyebrow">INSTAGRAM COMMUNITY</p><span className="platform-badge"><Icon name="instagram" size={13} />Instagram</span></div><div className="detail-title-line"><h1>{community.name}</h1>{community.verificationStatus === "verified" && <span className="detail-verified"><Icon name="check" size={13} />Verified</span>}</div><div className="tag-row">{community.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><p className="detail-description">{community.description}</p>{metadata.length > 0 && <div className="detail-metadata">{metadata.map((item) => <span key={item.label}><Icon name={item.icon} size={16} />{item.label}</span>)}</div>}<div className="detail-trust-line"><span>{community.healthLabel}</span><span>{community.listingAgeLabel}</span>{communityRow.last_verified_at && <span>Verified {new Intl.DateTimeFormat("en-IN", { dateStyle: "medium" }).format(new Date(communityRow.last_verified_at))}</span>}</div>{guidelines.length > 0 && <dl className="detail-guidelines">{guidelines.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>}{communityRow.join_enabled !== false ? <Link className="join-button detail-join" href={`/join/${community.slug}`}><Icon name="instagram" size={18} />Join on Instagram <Icon name="arrow" size={16} /></Link> : <span className="join-button detail-join join-disabled" aria-disabled="true"><Icon name="instagram" size={18} />Join temporarily unavailable</span>}<CommunityDetailActions slug={community.slug} name={community.name} /></div>
    </section>
    <CommunityDetailTrust memberCount={communityRow.member_count} language={communityRow.language} region={communityRow.region} ageRestriction={communityRow.age_restriction} eligibility={communityRow.eligibility} rules={communityRow.community_rules} restrictions={communityRow.restrictions} verificationStatus={communityRow.verification_status} healthLabel={community.healthLabel} lastVerifiedAt={communityRow.last_verified_at} />
