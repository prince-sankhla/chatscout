...
export default async function CommunityPage({ params }: PageProps) {
  ...
  return <PageShell><main className="page-content detail-page"><Link href="/" className="back-link">← Back to discovery</Link><section className="detail-hero detail-hero-polished detail-hero-compact"><div className={`detail-art detail-art-polished art-${community.accent}`}>{community.imageUrl ? <img className="community-image" src={community.imageUrl} alt={`${community.name} community`} /> : <><span className="art-orbit" /><span className="art-copy">{community.initials.split("\n").map((line) => <span key={line}>{line}</span>)}</span></>}</div><div className="detail-copy detail-copy-polished"><div className="detail-label-row"><p className="eyebrow">INSTAGRAM COMMUNITY</p><span className="platform-badge"><Icon name="instagram" size={13} />Instagram</span></div><h1>{community.name}</h1><div className="tag-row">{community.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><p className="detail-description">{community.description}</p>{metadata.length > 0 && <div className="detail-metadata">{metadata.map((item) => <span key={item.label}><Icon name={item.icon} size={16} />{item.label}</span>)}</div>}<Link className="join-button detail-join" href={`/join/${community.slug}`}><Icon name="instagram" size={18} />Join on Instagram <Icon name="arrow" size={16} /></Link><CommunityDetailActions slug={community.slug} name={community.name} /></div></section>{related.length > 0 && <Reveal><section className="discover-section detail-related"><div className="section-heading"><div><p className="eyebrow">DISCOVER MORE</p><h2>Related <span>Communities</span></h2></div><Link href="/categories">Browse categories <Icon name="arrow" size={14} /></Link></div><CommunityGrid communities={related} compact /></section></Reveal>}{trending.length > 0 && <Reveal><section className="discover-section detail-trending"><div className="section-heading"><div><p className="eyebrow">FRESH DISCOVERY</p><h2>Trending <span>GCs</span></h2></div><Link href="/trending">View all <Icon name="arrow" size={14} /></Link></div><CommunityGrid communities={trending} compact /></section></Reveal>}</main></PageShell>;
}
