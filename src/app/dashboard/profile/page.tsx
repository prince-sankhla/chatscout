    ...
    <section className={styles.section}>
      <article className={styles.card} style={{ maxWidth: 720 }}>
        <div style={{ display: "flex", gap: 16, alignItems: "center", padding: 18 }}>
          ...
          <div><h2 style={{ margin: 0, fontSize: 23 }}>{profile.name}</h2><p style={{ margin: "5px 0 0", color: "var(--muted)", fontSize: 12 }}>{profile.email ?? "Email unavailable"}</p><span className={`${styles.badge} ${styles.success}`} style={{ marginTop: 8, display: "inline-flex" }}>Community owner</span></div>
        </div>
