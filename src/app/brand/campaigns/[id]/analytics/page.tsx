...
export default async function BrandCampaignAnalyticsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: links } = await db.from('campaign_links').select('id,short_code,click_count,unique_click_count,community_id,allocated_budget:community_id,community:communities(name,member_count)').eq('campaign_id',id).order('created_at',{ascending:false});
